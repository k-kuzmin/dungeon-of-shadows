...
- [x] `World` — менеджер сущностей (создание, удаление, `QueryInto<T>()`, `QueryInto<T1,T2>()`)
...
- [x] Плавное движение (SPD = 5.5 тайл/с)
...
- **Скорость** — 5.5 тайл/с (актуальное значение в `GameConfig.PlayerSpeed`)
...
| `src/ECS/Core/ITickable.cs` | Интерфейс `ITickable` |
| `src/ECS/Core/ComponentStore.cs` | Генерик-хранилище компонентов |
| `src/ECS/Core/World.cs` | Entity manager, `QueryInto<T>()`, `QueryInto<T1,T2>()` |
| `src/ECS/Player/PlayerTag.cs` | Компонент игрока |
| `src/ECS/Physics/Position.cs` | Компонент позиции |
| `src/ECS/Physics/Velocity.cs` | Компонент скорости |
| `src/ECS/Physics/Collider.cs` | Компонент коллайдера |
| `src/ECS/Rendering/Sprite.cs` | Компонент визуального представления |
| `src/ECS/Player/InputSystem.cs` | WASD/стрелки, 8 направлений |
| `src/ECS/Physics/PhysicsSystem.cs` | AABB коллизии, скольжение |
| `src/ECS/Rendering/CameraSystem.cs` | Плавный lerp, screen-shake |
| `src/ECS/Rendering/RenderSystem.cs` | Обёртка рендера (BeginDrawing/EndDrawing) — расширена в Фазе 2 |
...
Запускаемое приложение с DI-архитектурой: окно 1280×720, 6 комнат с коридорами, игрок двигается WASD (5.5 тайл/с), скользящие коллизии со стенами, плавная камера, дебаг-режим F3.
